...
const OwnerDashboard = () => {
  ...
  const [overallBookings, setOverallBookings] = useState([]);
  ...
  const loadOverallBookings = async () => {
    setOverallLoading(true);
    try {
      const res = await bookingService.getBookingsByOwner(user.email);
      const list = res.success && Array.isArray(res.data) ? res.data : [];
      const all = list.map(b => ({
        id:       b.id,
        customer: b.playerName || b.playerEmail?.split('@')[0] || 'Guest',
        venue:    b.venueName || b.venue?.name || b.venueId || '—',
        date:     b.bookingDate || '',
        time:     `${(b.startTime || '').substring(0, 5)} - ${(b.endTime || '').substring(0, 5)}`,
        status:   b.status?.toLowerCase() || 'pending',
        amount:   b.amount || 0,
      ...
      setOverallBookings(all);
    } catch (e) {
      console.error('Overall bookings load error', e);
    }
    setOverallLoading(false);
  };

  // Auto-load overall bookings on mount and refresh every 60 seconds
  useEffect(() => {
    if (!user?.email) return;
    loadOverallBookings();
    const interval = setInterval(loadOverallBookings, 60000);
    return () => clearInterval(interval);
  }, [user?.email]);

  const handleTabChange = (tab) => {
    setActiveTab(tab);
    if (tab === 'overall') loadOverallBookings();
  };
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
              {/* Tab Header */}
              <div className="bg-linear-to-r from-gray-50 via-gray-100 to-gray-50 px-6 pt-5 border-b border-gray-200">
                  ...
                  <div className="flex gap-1 bg-gray-200 p-1 rounded-xl">
                    ...
                    >
                      ...
                      <span className={`px-1.5 py-0.5 rounded-md text-xs font-bold ${
                        activeTab === 'today' ? 'bg-primary-100 text-primary-700' : 'bg-gray-300 text-gray-600'
                      }`}>
                    ...
                    >
                      ...
                      Overall Bookings
                      <span className={`px-1.5 py-0.5 rounded-md text-xs font-bold ${
                          activeTab === 'overall' ? 'bg-primary-100 text-primary-700' : 'bg-gray-300 text-gray-600'
                        }`}>
                          {overallLoading ? '…' : overallBookings.length}
                        </span>
                    </button>
                  </div>
                  {activeTab === 'today' && (
                ...
                <p className="text-sm text-gray-600 pb-4">
                  {activeTab === 'today'
                    ...
                    : overallLoading
                      ...
                      : `${overallBookings.length} total bookings across all your venues`}
                </p>
              </div>

              {/* Today's Bookings Content */}
              {activeTab === 'today' && (
                <>
                  {loading ? (
              ...
              {activeTab === 'overall' && (
                <>
                  ...
                  ) : overallBookings.length === 0 ? (
                    <div className="py-12 text-center">
                      <Activity className="mx-auto text-gray-300 mb-3" size={48} />
                      <p className="text-gray-500 text-lg font-medium">No bookings found</p>
                      <p className="text-gray-400 text-sm mt-1">Bookings will appear here once players start booking your venues</p>
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className="bg-gray-50 border-b-2 border-gray-200">
                          <tr>
                            <th className="text-left py-4 px-6 font-bold text-xs text-gray-700 uppercase tracking-wider">Player Name</th>
                            <th className="text-left py-4 px-6 font-bold text-xs text-gray-700 uppercase tracking-wider">Booked Venue</th>
                            <th className="text-left py-4 px-6 font-bold text-xs text-gray-700 uppercase tracking-wider">Date</th>
                            <th className="text-left py-4 px-6 font-bold text-xs text-gray-700 uppercase tracking-wider">Slot Time</th>
                            <th className="text-left py-4 px-6 font-bold text-xs text-gray-700 uppercase tracking-wider">Status</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                          {overallBookings.map((booking, index) => (
                            <tr
                              key={booking.id}
                              className={`hover:bg-linear-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-150 ${
                                index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'
                            ...
                            >
                              <td className="py-4 px-6">
                                <div className="flex items-center">
                                  <div className="w-9 h-9 bg-linear-to-br from-primary-500 to-primary-600 rounded-xl flex items-center justify-center shadow-sm">
                                    <span className="text-white font-bold text-sm">
                                      {booking.customer.charAt(0).toUpperCase()}
                                    </span>
                                  </div>
